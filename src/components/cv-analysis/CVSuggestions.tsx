        ...
        <CardContent className="pt-6">
          <Tabs defaultValue="byPriority">
            ...
            <TabsContent value="byPriority" className="mt-0 space-y-6">
              {['critical', 'recommended', 'nice'].map((priority) => (
                <div key={priority} className="space-y-4">
                  ...
                  <ul className="space-y-4">
                    {prioritizedSuggestions[priority]?.map((suggestion, index) => (
                      <li key={index} className={`flex flex-col gap-2 p-4 rounded-lg ${categoryConfig[suggestion.category].bgColor}`}>
                  ...
                  <AccordionItem key={category} value={category}>
                    <AccordionTrigger className="py-4">
                      <div className="flex items-center gap-2">
                        ...
                        {categorizedSuggestions[category]?.some(s => s.priority === 'critical') && (
                    ...
                    <AccordionContent>
                      <ul className="space-y-4 pt-2">
                        {categorizedSuggestions[category]?.map((suggestion, index) => (
                          <li key={index} className={`flex flex-col gap-2 p-4 rounded-lg ${categoryConfig[suggestion.category].bgColor}`}>
                            <div className="flex justify-between items-start">
