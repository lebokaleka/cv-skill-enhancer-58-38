...
const TemplateSelector = ({
  ...
  return <Card className="glass-card border-dashed">
        ...
        <div className="flex justify-between gap-4 overflow-x-auto pb-2 px-[15px] my-0 py-[15px]">
          {displayTemplates.map(template => <div key={template.id} className={`relative flex-1 min-w-[120px] border rounded-lg p-2 transition-all duration-300 ${selectedTemplate === template.id ? 'border-primary ring-2 ring-primary/20 shadow-md transform scale-105' : 'border-border hover:border-primary/30 hover:shadow-sm'}`}>
              ...
              <div className="relative w-full aspect-[1/1.4] rounded-md overflow-hidden cursor-pointer mb-3" onClick={() => onSelectTemplate(template.id)}>
                {template.imageUrl ? <Image src={template.imageUrl} alt={template.name} className="object-cover w-full h-full transition-opacity duration-200" fallback="/placeholder.svg" /> : <div className="w-full h-full bg-gray-100 flex items-center justify-center text-gray-400">
                    <span className="text-xs">No preview</span>
                  </div>}
                
                {/* Selection Indicator */}
                {selectedTemplate === template.id && <div className="absolute top-2 right-2 bg-primary text-white rounded-full p-1 shadow-md">
                    <Check size={14} />
                  </div>}
              </div>
              ...
              <div className="w-full flex justify-center">
                <Button size="sm" variant={selectedTemplate === template.id ? "default" : "outline"} className={`w-full transition-all duration-300 ${selectedTemplate === template.id ? 'shadow-md' : 'hover:shadow-sm'}`} onClick={() => onSelectTemplate(template.id)}>
